...
class Burger {
    constructor (size, add, toppings) {
        this.size = new Param (this._select (size)) // {price: x, calories: y}
        this.add = new Param (this._select (add)) // {price: x1, calories: y1}
        this.toppings = this._getToppings (toppings) // [{price: x2, calories: y2}, {price: x3, calories: y3}]
    }
    _getToppings (name) {
        let result = []
